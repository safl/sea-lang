...
endef
.PHONY: all
all: uninstall clean build install example-enum docs
...
define example-struct-help
# Run using the example for struct
endef
.PHONY: example-struct
example-struct:
	slang examples/struct.h

define example-enum-help
# Run using the example for enum
endef
.PHONY: example-enum
example-enum:
	slang examples/enum.h
